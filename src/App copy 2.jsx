...
const T = {
  ...
  // ── Demand sub-types
  outside: '#1A97D2',   // Ocean  — زوار خارجيون
  inside: '#80C6E6',   // Rust   — زوار داخليون (brand color, unchanged)

  // ── Seasons — same gold hue, clearly distinct shades
  ...
  // ── Warning — Gold (consistent with primary)
    ...
    <div className={`sc-slider${isEmpty ? '' : ' sc-slider--on'}${isNeg ? ' sc-slider--neg' : ''}`}
      ...
      <div className="sc-sl-row">
        ...
        <span className="sc-sl-val" style={{
          ...
          borderColor: isEmpty ? 'rgba(255,255,255,0.07)' : isNeg ? T.deficitL : `${color}38`,
        }}>{pctStr(value)}</span>
      </div>
      {/* track */}
    ...
    <div className={`sc-slider${isChanged ? ' sc-slider--on' : ''}${isBelow && isChanged ? ' sc-slider--neg' : ''}`}
      ...
      <div className="sc-sl-row">
        ...
        <span className="sc-sl-val" style={{
          ...
          borderColor: !isChanged ? 'rgba(255,255,255,0.07)' : isBelow ? T.deficitL : `${color}38`,
      ...
      <div className="sc-sl-edge" dir="ltr"><span>{min}</span><span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 8 }}>افتراضي: {normal}</span><span>{max}</span></div>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        border: `1.5px solid ${visible ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.2)'}`,
      ...
      {visible && createPortal(
        <div style={{
          ...
          wordBreak: 'break-word',
          overflowWrap: 'anywhere',
          boxShadow: '0 10px 36px rgba(0,0,0,0.72), 0 2px 10px rgba(0,0,0,0.45)',
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <TT minW={180}>
        ...
        {item?.hijriByYear?.length > 0 && (
          <div style={{ marginBottom: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {item.hijriByYear.map(({ gYr, months }) => (
              <div key={gYr} style={{ display: 'flex', alignItems: 'baseline', gap: 6, fontSize: 10 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: T.bronzeXL, fontWeight: 800, flexShrink: 0 }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: T.bronzeXL, display: 'inline-block', flexShrink: 0 }} />
                  {gYr}
                </span>
                <span style={{ color: 'rgba(255,255,255,0.25)', flexShrink: 0 }}>|</span>
                <span style={{ color: T.txtSub }}>{months.join(' · ')}</span>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <ChartCard title="العجز أو الفائض الشهري"
      ...
      <ResponsiveContainer width="100%" height={160} minWidth={0}>
        <BarChart data={monthly} margin={{ top: 8, right: 3, left: -35, bottom: 2 }} barCategoryGap="28%">
          ...
          <YAxis type="number"
            ...
            width={46} />
          <ReferenceLine y={0} stroke="rgba(255,255,255,0.22)" strokeWidth={1.5} strokeDasharray="4 3" />
          <Tooltip
      ...
      headerExtra={
        <>
          <select
            ...
            {availMos.map(m => (
              <option key={m} value={m} style={{ background: '#1A1819' }}>{AR_MON[m]}</option>
            ))}
          </select>
          <button
      ...
      <div className="sidebar-header">
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ fontSize: 13, fontWeight: 900, color: '#F4F1EB', letterSpacing: '0.3px' }}>أداة ماذا لو؟</div>
          ...
          <div style={{ fontSize: 9.5, color: 'rgba(212,170,82,0.55)', marginTop: 3, fontWeight: 600 }}>محاكاة افتراضية للبيانات</div>
        </div>
      ...
      >
        <div
          ...
          style={{
            ...
            background: 'rgba(150,113,38,0.1)'
          }}
        >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr auto 1fr', gap: 8, alignItems: 'center' }}>
                    {[
                      ...
                      item.sym
                        ? <div key={i} style={{ fontSize: 20, color: 'rgba(255,255,255,0.2)', textAlign: 'center' }}>{item.sym}</div>
                        : (
                          <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${item.border}`, borderRadius: 12, padding: '12px', textAlign: 'center' }}>
                  ...
                  {/* الوحدات */}
                  <div style={{ borderRadius: 10, padding: '14px 16px', border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                      {[
                        ...
                        <div key={unit} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(0,0,0,0.12)', borderRadius: 7, padding: '8px 12px' }}>
...
function Dashboard({ db, fileDate }) {
  ...
  /* ══════════════════════════════════════ RENDER ══════════════════════════════════════ */
  return (
    <div className="cl-root" style={{ direction: 'rtl', background: '#151515', minHeight: '100vh', color: '#F4F1EB' }}>
          ...
          <div className="content-area">
                ...
                <div className="kpi-top-grid">
                  ...
                  <div className="card fade-up"
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      ...
                      {peakDemand && !peakDemand.isSingleDay && <span style={{ fontSize: 9.5, color: T.txtDim, fontWeight: 700, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', padding: '1px 7px', borderRadius: 10, whiteSpace: 'nowrap' }}>{peakDemand.days} يوم</span>}
                    </div>
                  ...
                  <div className="card fade-up"
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      ...
                      {peakSupply && !peakSupply.isSingleDay && <span style={{ fontSize: 9.5, color: T.txtDim, fontWeight: 700, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', padding: '1px 7px', borderRadius: 10, whiteSpace: 'nowrap' }}>{peakSupply.days} يوم</span>}
                    </div>
                    ...
                    return (
                      <div className="card fade-up"
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                          ...
                          {hasDef && kpi?.maxDef && !kpi.maxDef.isSingleDay && <span style={{ fontSize: 9.5, color: T.txtDim, fontWeight: 700, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', padding: '1px 7px', borderRadius: 10, whiteSpace: 'nowrap' }}>{kpi.maxDef.days} يوم</span>}
                        </div>
                  ...
                  {kpi?.maxSur?.value > 0 ? (
                    <div className="card fade-up"
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        ...
                        {kpi?.maxSur && !kpi.maxSur.isSingleDay && <span style={{ fontSize: 9.5, color: T.txtDim, fontWeight: 700, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', padding: '1px 7px', borderRadius: 10, whiteSpace: 'nowrap' }}>{kpi.maxSur.days} يوم</span>}
                      </div>
                    ...
                    <div className="card fade-up"
                      style={{ padding: '14px 13px', borderTop: `2px solid rgba(255,255,255,0.07)`, borderRadius: 12, opacity: 0.4 }}>
            ...
            {series.length > 0 && (
              <div className="span-full mini-charts-trio">
                ...
                <MonthlyBarChart monthly={mini.monthly} defPct={mini.defPct} series={series} yrs={yrs} />
              ...
              <div className="card fade-up" style={{ padding: '18px 16px 14px' }}>
                ...
                {series.length > 0 ? (
                  <>
                            ...
                            <Line type="monotone" dataKey="demand" name="demand" stroke={T.dem} strokeWidth={2.8} dot={false} animationDuration={1600} animationEasing="ease-out"
                              activeDot={({ cx, cy }) => (
                                <circle cx={cx} cy={cy} r={7} fill={T.dem} stroke="#1A1819" strokeWidth={2.5}
                                  style={{ filter: `drop-shadow(0 0 8px ${T.dem}80)` }} />
                              )} />
                    ...
                    {/* Legend strip */}
                    <div style={{ display: 'flex', gap: 18, justifyContent: 'center', marginTop: 14, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)', flexWrap: 'wrap' }}>
                      {[
                      ...
                      <div className="card fade-up" style={{ padding: '16px 18px 14px', position: 'relative' }}>
                        ...
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                            <span style={{ fontSize: 9, color: T.txtDim }}>نسبة تغطية الطلب</span>
